...
def neg_sampling_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    scores = np.dot(center_word_vec, outside_vectors[outside_word_idx].T)
    cols_for_neg_mat = np.arange(outside_vectors.shape[1])
    neg_samples_mat = outside_vectors[neg_sample_word_indices, cols_for_neg_mat]
    neg_samp_mult_with_vc = np.dot(neg_samples_mat, center_word_vec.T)
    loss = -1 * np.log(sigmoid(scores)) -np.sum(np.log(sigmoid(-1 * neg_samp_mult_with_vc)))
    grad_center_vec = -(1-sigmoid(scores))*outside_vectors[outside_word_idx] + np.sum((1-sigmoid(-1*neg_samp_mult_with_vc)).T * neg_samples_mat, axis=0)
    mat_of_vcs = np.tile(center_word_vec, (K, 1))
    grad_outside_vecs = (1-sigmoid(-1*neg_samp_mult_with_vc)).T * mat_of_vcs
    grad_outside_vecs[outside_word_idx] = grad_outside_vecs[outside_word_idx]*-1
    ### END YOUR CODE 􏰏Kk=1 (1 − σ(−uTk vc))uk.
